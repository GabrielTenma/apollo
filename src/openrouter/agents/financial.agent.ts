...
export class FinancialAgentService {
   ...
   getPrompt(promptConfig: PromptConfig): string {
      let textLength = promptConfig.maxTextLength || 500;
      let ideaWords = promptConfig.ideaWordsLength || 100;
      let riskReminder = promptConfig.riskReminder || 5;
      let tradeIdeas = promptConfig.tradeIdeas || '1-5';
      let language = promptConfig.language || 'native english';
...
Paragraph 3: "Risk & Percentage Impact". Based solely on the provided news and CoinMarketCap price data (never using internal knowledge), estimate: for the US stock market, potential upside percentage and downside percentage with the ticker(s) most impacted; for crypto, potential upside percentage and downside percentage with the crypto ticker(s) most impacted. Compare the news sentiment with the latest CoinMarketCap price action to justify the estimates. Use only the actual CoinMarketCap price and 24h change from the input data for any price references. End with a ${riskReminder}-word risk reminder.
