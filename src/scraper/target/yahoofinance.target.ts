...
export class YahooFinanceTarget {
    ...
    parseNewsItems(html: string): YahooNewsItem[] {
        ...
        $('ul.stream-items li.stream-item.story-item').each((_, element) => {
            ...
            // Title and link
            const $titleLink = $section.find('a.subtle-link.fin-size-small.titles.noUnderline');
            ...
            $section.find('div.taxonomy-links span.ticker-wrapper a.ticker').each((_, tickerEl) => {
