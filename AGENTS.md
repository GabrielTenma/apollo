...
| Task | Command |
|---|---|
| Dev (NestJS + Vite) | `npm run dev` |
| Dev (NestJS only) | `npm run start:dev` |
| Dev (debug) | `npm run start:debug` |
| Build backend | `npm run build` |
| Build frontend | `npm run web:build` |
| Build all | `npm run build:all` |
| Production start | `npm run start:all` (serves dist/main + dist/web) |
| Lint | `npm run lint` |
...
> NestJS serves on **http://localhost:3000**.  
...
| POST | `/login` | Public | — |
| POST | `/refresh` | Public | — |
| GET | `/admin-only` | JWT | `admin` |
| GET | `/moderator-or-admin` | JWT | `admin`, `moderator` |
| GET | `/profile` | JWT | any |

### `/api/v1/scraper`
| Method | Path | Auth | Note |
|---|---|---|---|
| POST | `/scrape` | JWT | Raw page scrape |
| POST | `/scrape-multiple` | JWT | Concurrent batch |
| POST | `/extract` | JWT | Structured extraction |
| POST | `/scrape-source/:id` | JWT | Active scraping source execution |
| GET | `/financialjuice` | JWT | Latest news from memory store |
| GET | `/yahoofinance` | JWT | Latest news from memory store |
| GET | `/coinmarketcap` | JWT | Latest prices from memory store |
| GET/POST/PUT/DELETE | `/sources` | JWT | ScrapingSourceEntity CRUD |
| GET/POST/PUT/DELETE | `/scraped-data` | JWT | ScrapedDataEntity CRUD |
| GET | `/scraped-data/result/latest/:size` | JWT | LLM result with ChatCompletion |

### `/api/v1/openrouter`
| Method | Path | Auth | Note |
|---|---|---|---|
| POST | `/chat` | `admin` / `moderator` | Full chat-completion |
| POST | `/simple-chat` | `admin` / `moderator` | Single-prompt |
| GET | `/models` | `admin` / `moderator` | List available models |
| GET | `/health` | Public | Health check |
| GET | `/completion` | Public | Latest + previous AI analysis |

### `/api/v1/telegram`
| Method | Path | Auth | Note |
|---|---|---|---|
| POST | `/webhook` | Public | Telegram webhook |
| POST | `/send-message` | JWT | Arbitrary send |
| POST | `/send-text` | JWT | Simpler text send |
| GET | `/bot-info` | JWT | getMe |
| POST | `/get-webhook` | JWT | Get webhook info |
| GET | `/health` | Public | Health check |

### `/api/v1/supabase`
| Method | Path | Auth | Note |
|---|---|---|---|
| POST | `/create` | JWT | Generic insert |
| GET | `/read/:table` | JWT | Generic select |
| PUT | `/update` | JWT | Generic update |
| DELETE | `/delete` | JWT | Generic delete |
| GET | `/health` | Public | Health check |
...
- **Unit / Integration:** Jest + ts-jest (`**/*.spec.ts` under `src/` and `test/`).
